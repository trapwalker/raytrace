...
module Surfaces {
    export var shiny: Surface = {
        ...
        reflect: function(pos) { return 1.0; },
        roughness: 250
    }
...
class RayTracer {
    ...
    render(scene, ctx, screenWidth, screenHeight) {
        var getPoint = (x, y, camera) => {
            ...
            var recenterY = y => - (y - (screenHeight / 2.0)) / 2.0 / screenWidth;
...
function exec(w: number, h: number) {
    var canv = <HTMLCanvasElement>document.createElement("canvas");
    canv.width = w;
    canv.height = h;
    document.body.getElementsByClassName("container")[0].appendChild(canv);
